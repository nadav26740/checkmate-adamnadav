...
bool Bishop::checkPath(int* newCord)
{
	...
	else if (abs(*newCord - this->_cords[0]) != abs(*(newCord + 1) - this->_cords[1]))// check if legal move
	{
		throw ChessEvents(INVALIED_PIECE_MOVE, "The Bishop Doing invalid move that he can't do!");
		return false;
	}
		...
		if (abs(*(newCord)-this->_cords[0]) == abs(*(newCord + 1) - this->_cords[1]))
		{
			...
			if ((*(newCord)-this->_cords[0]) - (*(newCord + 1) - this->_cords[1]))
			{
				for (int i = 1; i < abs(*(newCord)-this->_cords[0]); i++)
				{
					...
					{
						throw ChessEvents(INVALIED_PIECE_MOVE, "The Bishop jumping over a piece");
						return false;
					}
				}
			}
